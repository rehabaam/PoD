...
RCT_EXPORT_METHOD(play:(NSString *)url:(NSDictionary *) metadata) {
    ...
    NSURL *soundUrl;
    if ([fileLocation isEqualToString:@"Local"]){
        
        //Local Album image
        NSString *image = [[NSBundle mainBundle] pathForResource:albumUrlStr ofType:@"png"];
        albumUrl = [NSURL fileURLWithPath:image];
        NSLog(@"Path image: %@",image);
        NSLog(@"Path Album: %@",albumUrl);
        
        //Local Sorry audio
        NSString *audio = [[NSBundle mainBundle] pathForResource:url ofType:@"wav"];
        soundUrl = [NSURL fileURLWithPath:audio];
    }
    else{
